...
export const CampaignsContextProvider: ({
    ...
    const campaignsRequest = useCallback(
        (pageIndex: number, previousPageData?: CampaignResource[][]) => {
    ...
    const {
        data: campaignsData,
        size: campaignsDataSize,
        setSize: setCampaignsDataSize,
        mutate: refreshCampaigns,
        isLoading: isCampaignsLoading,
    } = useSWRInfinite(campaignsRequest);

    const storeCampaign = useCallback(
        async (data: StoreCampaignDataType) => {
            await api.post(
                route("workspaces.campaigns.store", {
                    workspaceId: currentWorkspace?.id,
                }),
                data,
            );
    ...
    const updateCampaignStatus = useCallback(
        async (campaignId: string, status: "active" | "paused") => {
            await api.patch(route("campaigns.update", { campaignId }), {
